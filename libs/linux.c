// from https://github.com/Fewbytes/rubber-docker
...
// method definitions exposed to python
...
PyMODINIT_FUNC PyInit_linux(void)
{
    ...
    PyModule_AddIntConstant(module, "CLONE_THREAD", CLONE_THREAD);      // thread
    ...
    PyModule_AddIntConstant(module, "MS_MOVE", MS_MOVE);                // Move a subtree
    PyModule_AddIntConstant(module, "MS_REC", MS_REC);                  // Recursive loopback
    PyModule_AddIntConstant(module, "MS_SILENT", MS_SILENT);            // Suppress the display of certain (printk()) warning messages in the kernel log
